  ...
  if (albuns){
    return (
      <div className="flex flex-wrap justify-center md:p-10 w-full">
        ...
        <Link to={`/album/${album.id}`} key={index} className="flex flex-col sm:w-1/2 md:w-1/3 lg:w-1/5 p-4 justify-center m-3 bg-gradient-to-tr from-slate-700 to-slate-800 rounded-md hover:shadow-envolve-xl cursor-pointer hover:scale-105 transition-all duration-1000">
          <img className="max-w-60" src={album.images[0].url} alt={album.name} />
