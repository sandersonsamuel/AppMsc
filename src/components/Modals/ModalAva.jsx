...
export function ModalAva({ albumInfos, alerta }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      doc(databaseApp, `avaliacoes/${auth.currentUser.uid}`),
      (docSnap) => {
        if (docSnap.exists()) {
  ...
  return (
    <>
      <i
        onClick={handleOpen}
        className="fa-solid fa-pen-to-square text-2xl md:text-3xl cursor-pointer"
      ></i>
          ...
          <div className="space-y-6">
            ...
            <div
              className={`w-12 border-2 ${
                limiteCaracteres - textAva.length <= 25
                  ? "border-red-500 text-red-500"
                  : limiteCaracteres - textAva.length > 25 &&
                    limiteCaracteres - textAva.length <= 50
                  ? "border-yellow-400 text-yellow-400"
                  : "border-green-500 text-green-500"
              } flex items-center justify-center rounded-full`}
            >
              <p className="">{limiteCaracteres - textAva.length}</p>
            </div>
          </div>
        </ModalBody>
        <Modal.Footer>
          <div className="flex w-full justify-between gap-1">
            <div className="flex gap-1 md:gap-2">
              <Button
                ...
                disabled={textAva.length > limiteCaracteres}
              >
                Enviar
              </Button>
